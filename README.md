...
## 담당 교사 이름 + 선택과목 그룹 문자 (선택 사항, 운영자 본인 학교 한정)

NEIS API는 담당 교사 이름과, 선택과목 이동수업의 그룹 문자(예: `E_역학과 에너지`의 `E`)를 제공하지 않아서 컴시간알리미 데이터를 별도로 가져와 보완합니다.
...
- `scripts/scrape_teachers.py`를 로컬에서 한 번 실행해보고 (comci 패키지 응답 필드명이 실제와 다를 수 있어 확인 필요) 정상 동작하면 `.github/workflows/update-teachers.yml`이 매일 자동 갱신합니다.
